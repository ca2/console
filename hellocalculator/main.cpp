...
string calculate(const string& str)
{
   ...
   auto pelementResult = parser.parse(str);

   auto result = pelementResult->get_result();

   string strResult = result.to_string();

   return strResult;

   //return "Don't know the answer yet";
...
void implement(::platform::context * pcontext)
{
   ...
   if (psubsystem->get_argument_count1() <= 0)
   {


      printf("please supply expression");
   ...
   for (int i = 0; i < psubsystem->get_argument_count1(); i++)
   {
      ...
      straExpression.add(psubsystem->get_argument1(i));

   }

   string strExpression = straExpression.implode(" ");

   printf("This is your query: %s\n", strExpression.c_str());

   string strResult = calculate(strExpression);

   printf("This is your answer: %s\n", strResult.c_str());
