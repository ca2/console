...
extern ::i8 * g_variables;

void NSISCALL pushstring(LPCTSTR str);
void NSISCALL pushintptr(INT_PTR value);
#define pushint(v) pushintptr((INT_PTR)(v))
int NSISCALL popstring(::i8 * str); // 0 on success, 1 on empty stack
int NSISCALL popstringn(::i8 * str, int maxlen); // with length limit, pass 0 for g_stringsize
...
::u32 NSISCALL myatou(LPCTSTR s); // converts a string to an ::u32eger, decimal only
int NSISCALL myatoi_or(LPCTSTR s); // with support for or'ing (2|4|8)
::i8 * NSISCALL getuservariable(const int varnum);
